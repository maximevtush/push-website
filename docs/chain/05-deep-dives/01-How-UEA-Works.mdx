...
Each UEA is linked to a user's origin wallet address. This is done by using the origin wallet address as the seed for the UEA's deterministic address generation. Whenever the UEA is deployed (always gasless) for a new user, a mapping is created between the origin wallet address and the UEA address. This mapping is stored on the [UEAFactory contract](/docs/chain/build/contract-helpers/#ueafactory-interface) and is accesible on-chain or through the SDK.

This mapping ensures that each UEA is linked to a user's origin wallet address and enables novel features such as chain tracking or enabling chain vs chain use cases. A great example of this is the [Ballsy App](https://ballsy.push.org) or the [Universal Counter Tutorial](/docs/chain/tutorials/basics/tutorial-universal-counter/#live-playground).
...
This mapping is stored on the [UEAFactory contract](/docs/chain/build/contract-helpers/#ueafactory-interface) and can be queried either **on-chain** or through the SDK. This ensures every user’s identity remains consistent across chains — their UEA always maps back to the same origin wallet.
