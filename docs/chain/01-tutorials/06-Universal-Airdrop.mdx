...
contract UniversalAirdrop is Ownable, ReentrancyGuard {
    ...
    function claim(
        ...
        require(token.transfer(caller, amount), "Token transfer failed");

        emit Claimed(caller, chainNamespace, chainId, amount);
    }
...
Copy `merkle-proof-generator/data/tree.json` and `merkle-proof-generator/data/addresses.json` into your web app’s static assets (for example, `frontend/public/`).
