...
```bash
npm install @pushchain/ui-kit @pushchain/core
```

## 1) Wrap your app with Push Universal Wallet
...
## 2) Build the Universal Payment UI

### 2.1) Hooks you'll use
...
### 2.2) Local state & derived amount
...
### 2.4) Connect Account Button and check connection status
...
### 2.5) Display Block Explorer URL
...
Open the printed local URL, connect a wallet or email, pick an amount, and hit **Send**.
