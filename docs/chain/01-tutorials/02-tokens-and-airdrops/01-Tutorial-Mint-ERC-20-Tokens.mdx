...
2. Mint `$WPUSH` tokens for liquidity and swaps
3. Create a `$AIRDROP/$WPUSH` pool and add liquidity
...
> **Important:** Copy the addresses you generated earlier into the `ADDRESSES` constant below.
> - `AIRDROP_ADDR`: from the `deployed-addresses.json` written by `scripts/deploy.ts` (Part 1)
> - `POOL_ADDR`: the `$AIRDROP/$WPUSH` pool address from Part 3.
...
Open `App.tsx` and replace it with the following implementation. Before pasting, update the `ADDRESSES` constant with the values copied from your `deployed-addresses.json` (see 4.4). This app:
