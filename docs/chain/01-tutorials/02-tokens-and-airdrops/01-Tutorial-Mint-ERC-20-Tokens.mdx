...
### 5.4. Add `deployed-addresses.json` to `public/`

Place the deployed addresses file in your frontend's `public` folder so the app can read your `$AIRDROP` at runtime:
...
### 5.5. Creating the Application Component
...
function PushSwapExample() {
  ...
  function Component() {
    ...
    const [airdropTokenAddress, setAirdropTokenAddress] = useState(null);
    ...
    useEffect(() => {
      (async () => {
        try {
          ...
          setAirdropTokenAddress(addr && addr.startsWith('0x') ? addr : null);
          const paddr = json?.POOL_ADDR;
          setPoolAddress(paddr && paddr.startsWith('0x') ? paddr : null);
        } catch {
