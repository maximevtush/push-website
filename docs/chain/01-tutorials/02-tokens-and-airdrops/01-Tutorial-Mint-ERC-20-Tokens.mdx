...
4. Build a minimal UI that allows users to mint `$WPUSH` tokens by depositing `$PC` tokens
...
Create a React app with Push Chain wallet integration to enable users to mint tokens by depositing `$PC` tokens.
...
try {
  const deployedData = JSON.parse(
    readFileSync('./deployed-addresses.json', 'utf8')
  );
  AIRDROP = deployedData.AIRDROP_ADDR;
  console.log(
    '📝 Using AIRDROP address from deployed-addresses.json:',
    AIRDROP
  );
} catch (error) {
  console.error(
    '❌ Could not read deployed-addresses.json. Please run deploy.ts first.'
  );
  process.exit(1);
}
...
function sqrtPriceX96FromHumanRatio(
  ...
  const baseUnitRatio = ratio
    .multipliedBy(new BN(10).pow(token1Decimals))
    .dividedBy(new BN(10).pow(token0Decimals));
...
async function main(): Promise<void> {
  ...
  if (!AIRDROP)
    throw new Error('AIRDROP address not found. Please run deploy.ts first.');
  ...
  if (pool === ethers.ZeroAddress) {
    ...
    for (const log of receipt.logs) {
      if ((log as any).address?.toLowerCase() !== FACTORY.toLowerCase())
        continue;
      try {
    ...
    const actualRatio: number =
      inputSymbol0 === sortedSymbol1 && inputSymbol1 === sortedSymbol0
        ? 1 / humanRatio
        : humanRatio;
  ...
  try {
    ...
    console.error(
      '❌ Failed to update deployed-addresses.json with POOL_ADDR',
      error
    );
  }
  ...
  const ratioBase = sqrtPriceX96BN
    .multipliedBy(sqrtPriceX96BN)
    .dividedBy(new BN(2).pow(192)); // token1_base per token0_base
  ...
  if (isWPUSH0) {
    ...
    const amount1Base = ratioBase
      .multipliedBy(amount0Base)
      .integerValue(BN.ROUND_FLOOR);
  ...
  const tickSpacing: number =
    poolFeeNum === 500 ? 10 : poolFeeNum === 3000 ? 60 : 200;
...
const EthersCompat = {
  ...
  formatUnits: (
    (ethers as any).formatUnits ?? (ethers as any).utils?.formatUnits
  ).bind(ethers),
  ZeroAddress:
...
async function findPool(
  ...
  for (const fee of fees) {
    ...
    if (addr && addr !== EthersCompat.ZeroAddress)
...
async function main(): Promise<void> {
  ...
  const { address: poolAddress, fee } = await findPool(
    factory,
    tokenInAddress,
    tokenOutAddress
  );
  ...
  console.log(
    `  ${symIn}  :`,
    EthersCompat.formatUnits(balInBefore, decIn),
    symIn
  );
  console.log(
    `  ${symOut} :`,
    EthersCompat.formatUnits(balOutBefore, decOut),
    symOut
  );

  // Approve router to spend tokenIn if needed (ethers v6: use target or getAddress)
  const routerAddr: string =
    (router as any).target ??
    (router as any).address ??
    (await (router as any).getAddress?.());
  ...
  console.log(
    `  ${symIn}  :`,
    EthersCompat.formatUnits(balInAfter, decIn),
    symIn
  );
  console.log(
    `  ${symOut} :`,
    EthersCompat.formatUnits(balOutAfter, decOut),
    symOut
  );
  console.log('Spent In   :', EthersCompat.formatUnits(spentIn, decIn), symIn);
  console.log(
    'Received Out:',
    EthersCompat.formatUnits(recvOut, decOut),
    symOut
  );
}
...
```bash
📝 Using AIRDROP address from deployed-addresses.json: 0x283E86104750B03Ed527Fa049193052778326122
🔄 PERFORMING SWAP
==================================================
...
## Part 5: Building the Frontend UI
...
### 5.1. Setting up Vite + React + TypeScript frontend
...
### 5.2. Installing `@pushchain/ui-kit`
...
### 5.3. Setting up the Push Chain Wallet
...
### 5.4. Creating the Application Component

Open `App.tsx` and replace it with the following mint-based implementation. This app:

- Integrates the Push Universal Wallet
- Lets users deposit `$PC` and mint `$WPUSH` by calling `deposit()` on the `WPUSH` contract
- Shows the transaction hash with a link to the explorer
      ...
      {isConnected && (
        <div style={{ marginTop: 16 }}>
          ...
          >
            ...
            <div style={{ marginTop: 8 }}>
              <button
                ...
                style={{
                  ...
                  padding: '8px 14px',
                  cursor:
                    !isConnected || isMintLoading || !mintAmount
                      ? 'not-allowed'
                      : 'pointer',
                }}
              >
            ...
            {mintError && (
              <div style={{ marginTop: 8, color: 'crimson' }}>{mintError}</div>
            )}
          </div>
        </div>
      )}
