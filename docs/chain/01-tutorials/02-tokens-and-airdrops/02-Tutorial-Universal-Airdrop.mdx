...
```
🔨 Building Merkle tree for 3 recipients...
...
- ✅ **TestToken contract** deployed and funded
- ✅ **UniversalAirdrop contract** deployed with Merkle root
- ✅ **Airdrop contract funded** with tokens for distribution
- ✅ **Deployment data saved** for frontend integration
...
Copy `merkle-proof-generator/data/tree.json` and `merkle-proof-generator/data/addresses.json` into your web app's static assets (in our case, `frontend/public/`).
...
<TabItem value='Generate Merkle Tree' label='Generate Merkle Tree'>
  ...
  <NodeJSVirtualIDE repo={{title: "Open in Github"}}>
  {`
    const { keccak256, getAddress, getBytes, dataSlice, toUtf8Bytes, toBeHex, zeroPadValue, concat } = ethers;

    function toEvmAddress(recipient) {
      if (recipient.startsWith('0x')) {
        return getAddress(recipient);
      }
      const rawBytes = Uint8Array.from(bs58.decode(recipient));
      const hashed = keccak256(rawBytes);
      const addrBytes = dataSlice(hashed, 12);
      return getAddress(addrBytes);
    }

    function generateLeaf(entry) {
      const { recipient, chainNamespace, chainId, amount } = entry;
      const evmRecipient = toEvmAddress(recipient);

      const recipientBytes = getBytes(evmRecipient);
      const namespaceBytes = toUtf8Bytes(chainNamespace);
      const chainIdBytes = toUtf8Bytes(chainId);
      const amountBytes = getBytes(zeroPadValue(toBeHex(BigInt(amount)), 32));
      const packed = concat([recipientBytes, namespaceBytes, chainIdBytes, amountBytes]);

      return keccak256(packed);
    }

    function buildMerkleTree(entries) {
      const leaves = entries.map(generateLeaf);
      ...
      return { merkleRoot, entries: entriesWithProofs };
    }
    ...
    function printSummary(treeData) {
      ...
      const chainStats = entries.reduce((acc, entry) => {
        const key = entry.chainNamespace + ':' + entry.chainId;
        if (!acc[key]) acc[key] = { count: 0, totalAmount: 0n };
        acc[key].count += 1;
        acc[key].totalAmount += BigInt(entry.amount);
        return acc;
      }, {});
      ...
      const totalAmount = entries.reduce((sum, e) => sum + BigInt(e.amount), 0n);
      console.log('\n💰 Total Airdrop Amount:', totalAmount.toString(), 'tokens');
    }
    ...
    async function main() {
      // Demo dataset
      const entries = [
        { recipient: '0x1234567890123456789012345678901234567890', chainNamespace: 'eip155', chainId: '11155111', amount: '1000000000000000000' },
        { recipient: '72JBejJFXrRKpQ69Hmaqr7vWJr6pdZXFEL6jt3sadsXU', chainNamespace: 'solana', chainId: 'EtWTRABZaYq6iMfeYKouRu166VU2xqa1', amount: '2000000000000000000' },
      ];
