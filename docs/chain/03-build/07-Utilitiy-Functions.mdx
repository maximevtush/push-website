...
| _`exponent`_  | `number` | The number of decimal places to scale by. Must be a non-negative integer. For example, use `18` for PC or ETH, `6` for USDC, `8` for BTC.              |

</PushAPIReference>

<Details summary="Returns `bigint`" className="alert alert--fn-args">
...
**_`PushChain.utils.helpers.slippageToMinAmount(amount, { slippageBps }): string`_**
...
**_`PushChain.utils.getMoveableTokens(chainOrClient?): { tokens: { chain, symbol, decimals, address }[] }`_**
...
<Details summary="Live Playground: List moveable tokens (all and Sepolia)" className="alert alert--live-play">
...
**_`PushChain.utils.getPayableTokens(chainOrClient?): { tokens: { chain, symbol, decimals, address }[] }`_**
...
<Details summary="Live Playground: List payable tokens (all and Solana Devnet)" className="alert alert--live-play">
