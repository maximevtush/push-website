...
**_`PushChain.utils.helpers.parseUnits(value, {options}): bigint`_**
...
Converts a raw token amount in smallest units (bigint) into a human-readable decimal string. It divides the given value by 10^decimals, making it easy to display amounts in human readable values.

Commonly used in UI or logs (e.g., turning `1500000000000000000` into `1.5` or any token from smallest unit to it's human readable value).
...
| _`options.decimals`_  | `number` | The number of decimal places to scale by. Must be a non-negative integer. For example, use `18` for PC or ETH, `6` for USDC, `8` for BTC.              |
| _`options.precision`_  | `number` | The number of precision to scale by, will round up the value. Must be a non-negative integer. For example, use `4` for returning 4 digits after the decimal.              |
...
import { PushChain } from '@pushchain/core';
    ...
    function main() {
      ...
      console.log('123456 with 6 decimal places and 4 precision (1.2346):', decimalResult.toString());
      
      console.log('=== Variation Examples ===');
      
      // Push token amount (18 decimals)
      const pushAmount = PushChain.utils.helpers.formatUnits('1000500000000000000000', 18);
      console.log('1000500000000000000000 uPC tokens to PC tokens (1000.5):', pushAmount.toString());
    }
...
```typescript
const chainName = PushChain.utils.chains.getChainNamespace(chainName);
```
...
```typescript
const chainName = PushChain.utils.chains.getChainName(namespace);
```
