...
| `tx.funds`                | `{ amount: BigInt; token?: MoveableToken }` | Bridges funds from the origin chain to Push Chain within the same call (move + execute when `tx.data` is provided). |
...
Multicall lets you execute multiple contract calls atomically within a single universal transaction. Instead of sending several transactions one-by-one, you supply an array of calls in the `tx.data` field, where each call includes `to`, `value`, and ABI-encoded `data`.

Key points:
- `tx.data` accepts an array of calls for multicall
- Each call: `to: 0x-prefixed address`, `value: bigint`, `data: 0x-encoded calldata`
- Executed atomically on Push Chain as part of one universal transaction
...
{`
    ...
    // Shape for a single multicall item (JS only)

    // ⭐️ MAIN FUNCTION ⭐️
    async function main() {
      console.log('🌟 Viem Multicall Example - Sepolia Origin → Push Chain Target');
...
## Bridge Funds with `sendTransaction`

Below are end-to-end examples showing how to bridge assets to Push Chain and execute your call in one step.

<Tabs className="liveplaytab" groupId="send-universal-funds-examples">
  <TabItem value='funds_payload' label='Funds + Payload (ERC-20, Sepolia)'>

  Passing the `funds` field bridges assets from your origin chain to Push Chain and then executes your call on Push Chain in one step. Pick an ERC-20 via `client.moveable.token.<SYMBOL>` and set `amount`; the SDK handles approvals and a small ETH deposit for Push gas.
  <br />
  Below, we call the Counter contract’s increment() from the Simple Counter tutorial ([01‑Tutorial‑Simple‑Counter](/docs/chain/tutorials/basics/tutorial-simple-counter/)), deployed on Push Testnet Donut. The SDK bridges ERC‑20 funds from Sepolia and executes the call on Push in one request.
  ...
  <TabItem value='funds_erc20_only' label='ERC-20 Funds Only'>

  Bridges an ERC-20 from Sepolia to your UEA on Push Chain with no calldata. Make sure the Sepolia account has ETH for gas and the ERC-20 you want to move.
  ...
  <TabItem value='funds_native_only' label='Native Funds Only'>

  Bridges the native asset from your origin chain to your UEA on Push Chain with no calldata. Ensure the Sepolia account has ETH for gas.
