...
<NodeJSVirtualIDE repo={{title: "Open in Github", url: "https://github.com/pushchain/push-chain-examples/tree/main/core-sdk-functions/initialize-push-chain-client"}}>
  ...
  async function main() {
      ...
      try {
        ...
        console.log('🛡️ Restricted call blocked: ' + msg);
      }
...
// Start with a writable client
const client1 = await PushChain.initialize(universalSignerEVM, {
  network: PushChain.CONSTANTS.PUSH_NETWORK.TESTNET_DONUT,
});

// 1) Reinitialize with the SAME signer → new instance, same addresses
const clientSame = await client1.reinitialize(universalSignerEVM, {
  network: PushChain.CONSTANTS.PUSH_NETWORK.TESTNET_DONUT,
});

// 2) Reinitialize with a DIFFERENT signer → new addresses
const client2 = await client1.reinitialize(universalSignerEVM2, {
  network: PushChain.CONSTANTS.PUSH_NETWORK.TESTNET_DONUT,
});

// 3) Reinitialize with a signer on a DIFFERENT CHAIN
const clientPush = await client1.reinitialize(universalSignerPush, {
  network: PushChain.CONSTANTS.PUSH_NETWORK.TESTNET_DONUT,
});

// 4) Reinitialize with new OPTIONS
const clientCustom = await client1.reinitialize(universalSignerEVM, {
  network: PushChain.CONSTANTS.PUSH_NETWORK.TESTNET_DONUT,
  rpcUrls: { [PushChain.CONSTANTS.CHAIN.ETHEREUM_SEPOLIA]: ['https://custom-sepolia.rpc.com'] },
