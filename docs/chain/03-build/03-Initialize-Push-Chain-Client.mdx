...
Use `pushChainClient.reinitialize(newSignerOrAccount, options)` to get a fresh `PushChainClient` with a different signer/account and/or updated configuration (RPCs, explorers, traces, progress hook).

<NodeJSVirtualIDE repo={{title: "Open in Github", url: "https://github.com/pushchain/push-chain-examples/tree/main/core-sdk-functions/initialize-push-chain-client"}}>

  {`
  // customPropHighlightRegexStart=\.reinitialize\(
  // customPropHighlightRegexEnd=}\);
  import { PushChain } from '@pushchain/core';
  import { privateKeyToAccount, generatePrivateKey } from 'viem/accounts';
  import { createWalletClient, http } from 'viem';
...
| Arguments                 | Type                               | Default     | Description |
| ------------------------- | ---------------------------------- | ----------- | ----------- |
| _`newSignerOrAccount`_    | `UniversalSigner \| UniversalAccount` | -           | Pass a `UniversalSigner` for full signing/tx capabilities, or a `UniversalAccount` to reinitialize in read-only mode. |
| _`options.network`_       | `PushChain.CONSTANTS.PUSH_NETWORK` | -           | Push Chain network to connect to (e.g., `PushChain.CONSTANTS.PUSH_NETWORK.TESTNET_DONUT`). |
| `options.rpcUrls`         | `Partial<Record<CHAIN, string[]>>` | `{}`        | Custom RPC URLs mapped by chain IDs. |
| `options.blockExplorers`  | `Partial<Record<CHAIN, string[]>>` | `{[CHAIN.PUSH_TESTNET_DONUT]: ['https://donut.push.network']}` | Custom explorer URLs mapped by chain IDs. |
| `options.printTraces`     | `boolean`                          | `false`     | Print internal trace logs for debugging. |
| `options.progressHook`    | `(progress: ProgressEvent) => void`| `undefined` | Optional callback to receive progress events. |
